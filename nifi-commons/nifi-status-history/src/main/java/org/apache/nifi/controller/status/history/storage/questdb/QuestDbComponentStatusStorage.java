...
abstract class QuestDbComponentStatusStorage<T> implements ComponentStatusStorage<T> {
    ...
    private final QuestDbEntityReadingTemplate<String, List<String>> readIdsTemplate
            = new QuestDbEntityReadingTemplate<>(QUERY_COMPONENT_IDS_TEMPLATE, record -> record.getSym(0).toString(), e -> e, e -> Collections.emptyList());
    ...
    public Collection<String> getIds() {
        return readIdsTemplate.read(dbContext.getEngine(), dbContext.getSqlExecutionContext(), Collections.singletonList(getTableName()));
    }
